...
arp_cache_delete(struct arp_cache *cache)
{
    ...
    debugf("del pa=%s ha=%s", ip_addr_ntop(cache->pa, addr1, sizeof(addr1)), ether_addr_ntop(cache->ha,addr2,sizeof(addr2)));
...
arp_resolve(struct net_iface *iface, ip_addr_t pa, uint8_t *ha)
{
    ...
    if (iface->dev->type != NET_DEVICE_TYPE_ETHERNET) {
        debugf("unsup hw addr type");
        return ARP_RESOLVE_ERROR;
    }
    if (iface->family != NET_IFACE_FAMILY_IP) {
        debugf("unsupp prot add type");
        return ARP_RESOLVE_ERROR;
    }
    mutex_lock(&mutex);
    cache = arp_cache_select(pa);
    ...
    mutex_unlock(&mutex);
    debugf("resolv, pa %s, ha %s", ip_addr_ntop(pa, addr1, sizeof(addr1)), ether_addr_ntop(ha,addr2,sizeof(addr2)));
